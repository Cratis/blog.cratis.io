...
- Demonstrated .NET projects target `net10.0`; resolve the current .NET 10 SDK and Chronicle releases under the demonstrated-stack rule in [`.cratis/ai/rules/project/blog-workflow.md`](.cratis/ai/rules/project/blog-workflow.md).
...
Distinguish **findings identified**, **fixes applied**, and **fixes verified** in the final status. A review report is not a repair, and a narrower follow-up must not silently close unrelated blockers. Reserve "ready for human review" for a post whose walkthrough and applicable checks have no unresolved blockers; otherwise name the remaining blockers or verification gaps.
