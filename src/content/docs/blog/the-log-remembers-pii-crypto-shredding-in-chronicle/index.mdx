...
But the approach Chronicle steers you toward is marking the *type*. Most personal data in a well-modeled domain already lives in concept types — `SocialSecurityNumber`, `DateOfBirth`, `EmailAddress` — rather than bare strings. Mark the concept once — here for a person's name:
