...
The [compliance documentation](https://cratis.io/chronicle/compliance/) walks through marking data, read-model interactions, the key lifecycle, and what a single erasure reaches. If you are new to Chronicle, the [.NET quickstart on this blog](/event-sourcing-in-dotnet-with-chronicle/) gets a store running locally in minutes from the official templates — mark a property `[PII]` on one of its events and inspect the stored event in Chronicle's bundled Workbench to see the mechanism with your own eyes. Everything described here is shipped in Chronicle 18.1 and MIT licensed, like the rest of the Cratis stack.
