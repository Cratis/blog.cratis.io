...
The fastest path is the [.NET quickstart on this blog](/event-sourcing-in-dotnet-with-chronicle/), which scaffolds a full-stack Cratis application from the official templates and appends real events through a handful of Arc commands. Add a `[Unique]` attribute to one of its events, append twice with the same value, and read the second append result — the whole mechanism in two requests. The [constraints documentation](https://cratis.io/chronicle/constraints/) covers the declarative form, violation messages, and releasing values, and the [Dynamic Consistency Boundary](https://cratis.io/chronicle/dynamic-consistency-boundary/) page sets constraints in their wider context alongside concurrency scopes.
