...
The simplest way to declare one is directly on the event type — this is a real, working equivalent in every client except TypeScript:
...
TypeScript does not support this workflow yet — use the declarative `IConstraint` form below.
</TabItem>
</Tabs>
...
<TabItem label="Elixir">
Elixir does not support this workflow yet — it has no declarative-class style; see the model-bound macros above for its releasing equivalent.
</TabItem>
<TabItem label="TypeScript">
```typescript
...
There is also a language boundary worth knowing, and it runs the other way from what you might guess. Model-bound attributes exist in four of the five clients — C#, Kotlin, Java, and Elixir all have a real `[Unique]`/`@Unique`/`unique(...)` equivalent — and TypeScript is the one client without it, working through the declarative style only. Flip to the declarative `IConstraint` style and the gap moves: C# and TypeScript have the full API including `RemovedWith`, while Kotlin and Java's `IConstraint` is narrower — no releasing a held value, no custom constraint names, no chaining a check across multiple event types. Elixir has no declarative-class style at all; its model-bound macros are the only way in. The enforcement is identical everywhere — it lives in the kernel — but the sugar differs per client, and the [documentation is explicit about the differences](https://cratis.io/chronicle/constraints/model-bound/).
